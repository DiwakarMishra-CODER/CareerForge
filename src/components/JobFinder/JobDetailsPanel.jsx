...
const JobDetailsPanel = ({ job, isOpen, onClose, isSaved, onSave }) => {
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed right-0 top-0 h-full w-full max-w-2xl bg-slate-950/95 backdrop-blur-2xl border-l border-slate-800 z-[101] overflow-y-auto"
                    >
                            ...
                            <div className="flex justify-between items-center">
                                <button onClick={onClose} className="p-3 bg-slate-900/50 hover:bg-slate-800 transition-colors rounded-full text-slate-400 hover:text-white border border-slate-800">
                                 ...
                                 <div className="text-right">
                                     ...
                                     <h4 className="text-5xl font-['Playfair_Display'] text-cyan-400 italic">{job.matchScore}%</h4>
                                 </div>
                             </div>
                             ...
                             <header className="space-y-6">
                                 <motion.div 
                                     initial={{ width: 0 }} 
                                     animate={{ width: 48 }} 
                                     className="h-1 bg-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.3)]" 
                                 />
                             ...
                             <section className="space-y-8">
                                 <h4 className="text-[11px] font-black text-slate-200 uppercase tracking-[0.3em] flex items-center gap-4">
                                     <Target size={18} className="text-cyan-400" /> The AI Synthesis
                                 </h4>
                                 <div className="p-10 bg-slate-900/30 border border-slate-800 rounded-sm relative overflow-hidden group">
                                     <div className="absolute top-0 left-0 w-1 h-full bg-cyan-400/30" />
                             ...
                             <section className="space-y-8">
                                 ...
                                 <div className="space-y-3">
                                     {job.skillsRequired.map(skill => (
                                         <div key={skill.name} className="flex justify-between items-center bg-slate-900/20 p-5 border border-slate-800/50 hover:border-cyan-500/30 transition-all duration-500">
                             ...
                             <section className="pt-12 border-t border-slate-800/50 space-y-10">
                                 ...
                                 {pitch && (
                                     ...
                                     >
                                         ...
                                         {pitch}
                                         <motion.span animate={{ opacity: [1, 0] }} transition={{ duration: 0.8, repeat: Infinity }} className="inline-block w-2 h-5 bg-cyan-400 ml-1" />
                                     </motion.div>
                                 )}
                             </section>
                             ...
                             <div className="flex gap-4 pt-4">
                                 <button 
                                     onClick={onSave}
                                     className={`flex-1 py-7 font-black uppercase tracking-[0.3em] text-xs transition-all transform active:scale-[0.98] border ${
                                         isSaved 
                                             ? 'bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20' 
                                             : 'bg-transparent border-slate-800 text-slate-400 hover:border-cyan-400 hover:text-cyan-400'
                                     }`}
                                 >
                                 ...
                                 <a 
                                     ...
                                     className="flex-1 py-7 bg-cyan-500 text-black text-center font-black uppercase tracking-[0.3em] text-xs hover:bg-white transition-all transform active:scale-[0.98] shadow-[0_20px_40px_rgba(6,182,212,0.2)] flex items-center justify-center gap-2"
                                 >
                                     Apply Direct <ArrowRight size={14} />
                                 </a>
                             </div>
                        </div>
                    </motion.div>
